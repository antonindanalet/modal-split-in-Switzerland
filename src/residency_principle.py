...
def run_residency_principle_by_agglomeration():
    ...
    and 2015 (year) and for two different definitions of agglomerations, the definition of 2000 and the definition of
    ...
    """
    print('Computing modal split by agglomeration with MTMC data 2021...')
    get_modal_split_by_agglomeration(mtmc_year=2021, agglo_def=2012, group_of_modes=False, percentage=False)
    get_modal_split_by_agglomeration(mtmc_year=2021, agglo_def=2012, group_of_modes=True, percentage=False)
    get_modal_split_by_agglomeration(mtmc_year=2021, agglo_def=2012, group_of_modes=True, percentage=True)
    get_modal_split_by_agglomeration(mtmc_year=2021, agglo_def=2012, group_of_modes=False, percentage=True)
...
def get_modal_split_by_commune_numbers(list_of_commune_numbers, mtmc_year, group_of_modes, percentage):
    ...
    if group_of_modes:
        if mtmc_year == 2015 or mtmc_year == 2021:
...
def get_modal_split_by_agglomeration(mtmc_year, agglo_def, group_of_modes, percentage):
    if mtmc_year == 2021:
        folder_path_output = Path('../data/output/2021/')
        identification_columns = ['HHNR']
    elif mtmc_year == 2015:
        folder_path_output = Path('../data/output/2015/')
        identification_columns = ['HHNR']
...
def get_type_of_commune():
    print('Warning: using always the same typology of communes (9 levels), independently of the year. '
          'Should be checked')
